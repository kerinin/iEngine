...
The first challenge is the selection of \( \mathcal{T} \), the intervals used to create the estimates \( \Phi \).  We treat each observation as a \( t_n \) and select a set of indicative \( t_n \) using the One-Class SVM algorithm with a covariance-based kernel function.

The One-Class SVM algorithm is commonly used for anomoly detection and clustering.  It operates by selecting the minimal enclosing hypersphere in the image space defined by some kernel function, usually the Radial Basis Function kernel.  Our objective is to use this algorithm to isolate an indicative set of windows over a set of data points; rather than using vectors as the input to the SVM, we shall use sets of vectors with arbitrary and heterogenous sampling in the time domain.  We must choose a kernel which expresses similarity between two time windows.

For our kernel we choose the covariance value of the observations in the two windows.  Covariance is a good candidate for use as a kernel, it is bilinear, symmetric, and positive semidefinite.  While it is limited by the assumption of normally distributed noise, we are using the One-Class SVM to quickly eliminate redundancy, so this lack of robustness seems an acceptable compromise.  The kernel function is defined as such:

\begin{equation} \label{eq:KOneClass} 
K_\theta( S_{t_i,\theta}, S_{t_j,\theta} ) = 1 - \frac{\text{Cov}( S_{t_i,\theta}, S_{t_j,\theta} )}{2}
\end{equation}

This kernel will be 0 of the two sets are perfectly correlated and 1 if they are perfectly anticorrelated.  This produces a positive definite kernel matrix.  We would like to eliminate the parameter \( \theta \) from the algorithm, so we use a range of \( \theta \) values in the optimization task.  We can take to approaches to this; the first is to scale \( S_{t,\theta} \) to some uniform range, say \( 0 \le t \le 1 \).  The second is to leave \( S_{t,\theta} \) unscaled and define a second kernel function for comparing heterogenous values of \( \theta \).  We shall use the latter method, with our kernel defined as the sum of the autocovariance over all offset values:

\begin{equation} \label{eq:KOneClassAuto}
K( S_{t_i,\theta_n}, S_{t_j,\theta_m} ) = \sum_p1 - \frac{ \text{Cov}( S_{p,\theta_n}, S_{t_j,\theta_m} ) }{ 2 }
\end{equation}
...
One possiblity is using the Mutual Information (which is essentially what the existing system uses).  For this to be an improvement we would need some method of evaluating the mutual information without evaluating the marginal probabilities of the subsets, which wikipedia suggests is possible: "Using the ideas of Kolmogorov complexity, one can consider the mutual information of two sequences independent of any probability distribution".  This approach requries a method of compression, and the Mutual Information is determined as the difference in data required.  The benefit of mutual information is that it abstracts the existing system to the point that it could use any compression algorithm, potentially enabling a more robust or computationally efficient approach to the data.
\end{quotation}
...
\bibitem{you}
	Chang Huai You et al
	\emph{An SVM Kernel With GMM-Supervector Based on the Bhattacharyya Distance for Speaker Recognition}
	2009
	\url{http://www1.i2r.a-star.edu.sg/~hli/papers/SPL_Vol16No1_GUMI_2009.pdf}

\bibitem{moreno}
	Pedro J. Moreno et al
	A Kullback-Leibler Divergence Based Kernel for SVM Classification in Multimedia Applications
	\url{http://www.svcl.ucsd.edu/publications/conference/2003/nips03/nips03.ps}
\bibitem{ruping}
	Stefan Ruping
	\emph{SVM Kernels for Time Series Analysis}
	2001
	\url{http://www-ai.cs.uni-dortmund.de/EVENTS/FGML2001/FGML2001-Paper-Rueping.pdf}
