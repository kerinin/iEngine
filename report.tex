...
The first challenge is the selection of \( \mathcal{T} \), the intervals used to define subsets \( S_{t,\theta} \).  Using the Support Vector approach, we search for a solution which represents the observed data as a linear combination of a Support Vectors drawn from the observed data.  Rather than using the observations \( X = [ (x_0,t_0),...,(x_\ell,t_\ell) ] \), we use the subsets \( S = [ S_{t,\theta} | t \in X, \theta \in \Theta ] \), where \( \Theta \) is set of values chosen as part of the algorithm.  We select Support Vectors from \( S \) to minimize the number of points required to describe the observed data \( X \) to some level of accuracy while maximizing the information entropy of the Support Vectors themselves.  We represent the optimization task as such:

% We need to build from OUR objectives to these - the relationship isn't clear
...
Because we are using vector sets rather than vectors, we must define a kernel capable of expressing similarity between vector sets. We choose the covariance value of the observations in the two sets.  Covariance is a good candidate for use as a kernel; it is bilinear, symmetric, and positive semidefinite.  While it is limited by the assumption of normally distributed noise, the interval selection process is intended only to reduce the dimensionality of the observed data, so this lack of robustness seems an acceptable compromise.  The kernel function is defined as such:

\begin{equation} \label{eq:KOneClass} 
K_\theta( S_{t_i,\theta}, S_{t_j,\theta} ) = 1 - \frac{\text{Cov}( S_{t_i,\theta}, S_{t_j,\theta} )}{2}
\end{equation}

This kernel will be 0 if the two sets are perfectly correlated and 1 if they are perfectly anticorrelated.  This produces a positive definite kernel matrix.  We would like to eliminate the parameter \( \theta \) from the algorithm, so we use a range of \( \theta \) values in the optimization task.  We can take to approaches to this; the first is to scale \( S_{t,\theta} \) to some uniform range, say \( 0 \le t \le 1 \).  The second is to leave \( S_{t,\theta} \) unscaled and define a second kernel function for comparing heterogenous values of \( \theta \).  We shall use the latter method, with our kernel defined as the sum of the autocovariance over all offset values:
