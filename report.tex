...
\begin{equation}
I(S_i,S_j) = K( S_i ) - K( S_i | S_j)
\end{equation}

We use this function to define a kernel which assigns high values to sets which are similar and low values to sets which are different (the denominator is used to avoid a singuarity if \( I = 0 \) ):

\begin{equation}
K(S_i,S_j) = \frac{1}{1+ I( S_i, S_j ) }
\end{equation}
...
% See if we can tweak this kernel into the covariance 
