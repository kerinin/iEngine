...
\varphi(\mathbf{x}, S) = \frac{1}{|\mathcal{S}|} \sum_{i \in \mathcal{S}} K_\gamma ( \{ S \cup \mathbf{x} \}, S_i )
\end{equation}
...
This requires the following minor extension of \ref{eq:KRBFparzen}, \ref{eq:PrSParzenMultiple} and \ref{eq:VarphiParzenSingle}:

\begin{equation} \label{eq:KRBFParzenMultiple}
K_\gamma( S_i, S_j ) =  \frac{1}{\gamma \sqrt{2\pi} } e^{-\frac{1}{\gamma} \|S_i - S_j \|_{KL}^2 }
\end{equation}

\begin{equation} \label{eq:PrSParzenMultiple}
\Pr( \mathcal{S} = S ) \simeq \sum_{i \in \mathbf{S}} \frac{1}{|\mathbf{S}|} K(S, S_i)
\end{equation}

\begin{equation} \label{eq:VarphiParzenMultiple}
\varphi(\mathbf{x}, S) = \frac{1}{|\mathbf{S}|} \sum_{i \in \mathbf{S}} K_\gamma ( \{ S \cup \mathbf{x} \}, S_i )
\end{equation}
