...
\Pr( \mathcal{S} = S ) &\simeq \frac{1}{|\mathcal{S}| \gamma} \sum_{t,\theta} K \left( \frac{ \left\| S - S_{t,\theta} \right\|_{KL} } { \gamma } \right)
\end{align}
...
\phi_n( \mathbf{x} ) = \frac{1}{|S_n| \gamma} \sum_{\mathbf{x}_i \in S_n} K \left( \frac{ \| \mathbf{x} - \mathbf{x}_i \|^2 }{ \gamma } \right)
\end{equation}
...
\varphi(\mathbf{x}, S) = \frac{1}{|\mathcal{S}| \gamma} \sum_{t,\theta} K \left( \frac{ \left\| \{ S \cup \mathbf{x} \} - S_{t,\theta} \right\|_{KL} } { \gamma } \right)
\end{equation}
...
\Pr( \mathcal{S} = S ) &\simeq \frac{1}{|\mathbf{S}| \gamma} \sum_{t,\theta} K \left( \frac{ \left\| S - S_{t,\theta} \right\|_{KL}^2 } { \gamma } \right)
\end{align}

\begin{equation} \label{eq:VarphiParzenMultiple}
\varphi(\mathbf{x}) = \frac{1}{ | \mathbf{S} | \gamma } \sum_{t,\theta} K \left( \frac{ \left\| \{ S \cup \mathbf{x} \} - S_{t,\theta} \right\|_{KL}^2 } { \gamma } \right)
\end{equation}
...
\varphi( \mathbf{x}, S) &= \sum_{i} \beta_i K_\gamma( S_\mathbf{x}, S_i ) \\
...
\begin{equation}
\| X - \phi \|_{SV} = \sum_{x \in X} \phi(x) \log \phi(x)
\end{equation}

Recalling that \( S_\mathbf{x} = \{ S \cup \mathbf{x} \} \), we observe that:

\begin{equation}
\| S_{\mathbf{x}} - S_i \|_{SV} = \| S - S_i \|_{SV} + \| \mathbf{x} - S_i \|_{SV}
\end{equation}

We restate the kernel function as:

\begin{equation}
K_\gamma( X, S_i ) = e^{ -\frac{ \| X - S_i \|_{SV} }{ \gamma } }
\end{equation}

In situations where probabilities are being calculated for a uniform set of points in \( \Omega^{\mathcal{S}} \), this can considerably reduce the computational time needed to evaluate \( \mathbf{K} \), as \( \|\mathbf{x},S_i \| \) can be computed once and then added to each \( \|S,S_i\|, S \in \mathcal{S} \).
